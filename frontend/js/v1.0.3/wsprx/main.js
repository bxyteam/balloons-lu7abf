...
async function initializeApp() {
  ...
  try {
    ...
    if (!result.error && result.haylista) {
      ...
      console.log('⚠ No Telemetry 1 data available to display');
      document.getElementById("telemetry1-table").innerHTML = `<p style="font-weight: 600; color: #2d3748;font-size: 14px;padding: 0 12px;">No telemetry 1 data available. Telemetry 1 skipped.</p>`;
    }
    ...
    if (AppState.config.balloonid && AppState.config.balloonid !== "") {
      ...
      if (!result2.error) {
        renderTelemetry2TablePaginated();
         ...
         renderTelemetry2TablePaginated({ messageNoData: "Error processing Telemetry 2 data" });
      }
    ...
    if (window.L) {
      try {
        if (AppState.map.beacon1.length > 0) {
          initializeMap();
          initializeUI();
        } else {
          setNoChartsMapLayout();
        }
      } catch (error) {
...
function populateLinkRef() {
  ...
  document.getElementById('dxLinkHamburger').href = dxhref;
  document.getElementById('dxLinkHamburger').classList.remove('hidden');
  ...
  if (AppState.config.SSID) {
    ...
    document.getElementById('chartLinkHamburger').href = href;
    document.getElementById('chartLinkHamburger').classList.remove('hidden');
  }
  ...
  document.getElementById('slowerLinkHamburger').href = slowerhref;
}
