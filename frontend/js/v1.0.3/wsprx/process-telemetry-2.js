...
function renderTelemetry2TablePaginated({ containerId = "telemetry2-table", defaultRowsPerPage = 100,  messageNoData = "No telemetry data available. No balloon ID specified. Telemetry 2 skkipped." } = {}) {
  const container = document.getElementById(containerId);

  if (!Array.isArray(AppState.telemetry.tele2RowData) || !AppState.telemetry.tele2RowData.length) {
    container.innerHTML = `<p style="font-weight: 600; color: #2d3748;font-size: 14px;padding: 0 12px;">${messageNoData}</p>`;
    return;
  }
